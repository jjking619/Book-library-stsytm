...
class MyWidget(QtWidgets.QWidget): 
    ...
    def initDatabase(self):
         self.con= sqlite3.connect("jka.db")    
    ...
    def onReturnBorrow(self):
        ...
        for row in selected_rows:
            ...
            return_borrowed=f"SELECT * FROM t_borrow WHERE user_id='{self.login_user_id}' AND book_isbn='{isbn}' AND return_date is not NULL;"
            self.cursor.execute(return_borrowed)
            returned_check = self.cursor.fetchone()
            if returned_check is not None:
                QtWidgets.QMessageBox.critical(self, "提示", "该书已归还！")
                return
            else:   
